...
export function CodeBlockNodeView(props: ReactNodeViewProps<HTMLElement>) {
  ...
  const copy = async () => {
    try {
      ...
      logger.error("copy code failed", e);
    }
  };
  ...
  return (
    <NodeViewWrapper
      data-drag-handle
      className={"relative my-3 overflow-hidden border border-neutral-200 " + (selected ? "ring-2 ring-brand-500" : "")}
    >
      <div className="flex items-center justify-between border-b border-neutral-200 bg-[#F8FAFF] px-2 py-1">
        ...
        <button
          className="flex items-center gap-1 px-1.5 py-0.5 text-[11px] text-neutral-500 hover:bg-neutral-200"
          onClick={copy}
        >
      ...
      <pre
        ref={preRef}
        className="m-0 overflow-x-auto whitespace-pre bg-[#F8FAFF] p-3 text-[12.5px] leading-1.6 text-neutral-700"
      >
        <NodeViewContent />
      </pre>
    </NodeViewWrapper>
