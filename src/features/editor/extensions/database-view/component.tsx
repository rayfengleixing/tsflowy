...
export function DatabaseViewNodeView(props: ReactNodeViewProps<HTMLElement>) {
  ...
  return (
    <NodeViewWrapper
      data-drag-handle
      className={
        "my-3 overflow-hidden rounded-lg border border-neutral-300 " + (selected ? "ring-2 ring-brand-500" : "")
      }
    >
          ...
          <thead>
            <tr>
              {fields.map((f) => (
                <th
                  key={f.id}
                  className="border-b border-r border-neutral-200 bg-neutral-100/50 px-2 py-1 text-left text-[11px] font-medium text-neutral-500"
                >
                  {f.name}
                </th>
              ))}
